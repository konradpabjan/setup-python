...
- Create an env variable called `AGENT_TOOLSDIRECTORY` and set it to `/opt/hostedtoolcache`. This is used to control where Python gets set up and installed.
  - In the same shell that your runner is using, type `AGENT_TOOLSDIRECTORY=/opt/hostedtoolcache`
  - A more permanent way of setting this env variable is to create a `.env` file in the same directory as your runner and adding `AGENT_TOOLSDIRECTORY=/opt/hostedtoolcache`. This ensures the variable is always set if you have configured your runner as a service.
- It is not possible to start the Linux runner with `sudo` and `/opt` usually requires root privileges to write to. To get around this you can change certain permissions.
  - Create a directory called `hostedtoolcache` inside `/opt`.
  - Change Permissions using `chown`
    - Change the user and group to be the same as the runners. You can get this information by using the `ls -l` command.
...
- Set `AGENT_TOOLSDIRECTORY` to `/Users/runner/hostedtoolcache.
